...
const MIDI_RANGE = [12, 115];

function constrainNoteRange(midiNoteNum) {
	if (midiNoteNum < MIDI_RANGE[0] || midiNoteNum > MIDI_RANGE[1]) console.log('** constrain **'); // test to see if this breaks anything
	while (midiNoteNum < MIDI_RANGE[0]) midiNoteNum += 12;
	while (midiNoteNum > MIDI_RANGE[1]) midiNoteNum -= 12;
	return midiNoteNum;
}
...
function getMelody(melody, startNote) {
	return melody.map(note => {
		if (note === null) {
			...
			return MIDI[constrainNoteRange(midiNoteNum)];
		}
	});
}
